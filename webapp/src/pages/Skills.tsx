...
import { fetchSkills, fetchSkillContent } from "@/lib/api";

function renderMarkdown(text: string) {
  return text
    .split("\n")
    .map((line) => {
      if (line.startsWith("### ")) return `<h3 class="text-base font-semibold text-zinc-100 mt-4 mb-2">${line.slice(4)}</h3>`;
      if (line.startsWith("## ")) return `<h2 class="text-lg font-semibold text-amber-500 mt-5 mb-2">${line.slice(3)}</h2>`;
      if (line.startsWith("# ")) return `<h1 class="text-xl font-bold text-zinc-100 mt-5 mb-3">${line.slice(2)}</h1>`;
      if (line.startsWith("- ")) return `<li class="text-sm text-zinc-400 ml-4 list-disc">${line.slice(2)}</li>`;
      if (line.startsWith("> ")) return `<blockquote class="border-l-2 border-amber-500/50 pl-3 text-zinc-500 italic text-sm my-2">${line.slice(2)}</blockquote>`;
      if (line.startsWith("```")) return `<pre class="bg-zinc-800 rounded-lg p-3 text-xs text-zinc-300 overflow-x-auto my-2 font-mono">`;
      if (line.trim() === "") return "<br />";
      if (/^`[^`]+`$/.test(line.trim())) return `<code class="bg-zinc-800 px-1.5 py-0.5 rounded text-xs text-amber-400 font-mono">${line.trim().slice(1, -1)}</code>`;
      return `<p class="text-sm text-zinc-400 my-1">${line}</p>`;
    })
    .join("\n");
}
